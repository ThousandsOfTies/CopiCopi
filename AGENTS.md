...
このファイルは `D:\Yurufuwa\CopiCopi` 以下に適用する。
CopiCopiは、お手本の模写・AI評価・Progressを扱う、TutoTuto派生のイラスト練習アプリ。

- メタリポジトリ：このディレクトリ（`main`）
- アプリ：`repos/copicopi-app`（`main`）
- 共通UI・PDF表示・保存・認証：`repos/home-teacher-common`（`main`）
- 描画基盤：`repos/drawing-common`（`main`）

旧 `C:\VibeCode` のパスを使用しない。
CopiCopi固有の模写評価・先生・Progress機能をTutoTutoへ自動的に取り込まない。
...
- CopiCopi固有の変更は `repos/copicopi-app` に入れる。
...
現行の課金情報はCopiCopi専用Firebaseプロジェクトの `users/{uid}` 直下に保存する。旧 `entitlements.copicopi` は使用しない。

## 起動・デプロイ

- フロント：メタで `make dev`、または `repos/copicopi-app` で `npm run dev`（Vite、既定3000）。
- API：メタで `make dev-server`、または `repos/copicopi-app/server` で `npm run dev`（Express、既定3003）。
- APIキーはサーバー側のみ。`VITE_API_URL` はAPIのベースURLで、末尾に `/api` を付けない。
- 環境変数は `repos/copicopi-app/.env.example` と `repos/copicopi-app/server/.env.example` を参照し、秘密情報をGitへコミットしない。
